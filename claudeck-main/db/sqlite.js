...
const stmts = {
  ...
  getSingleSessionCosts: db.prepare(
    `SELECT s.id, s.title, s.project_name, s.last_used_at,
            COALESCE(SUM(c.cost_usd), 0) AS total_cost,
            COALESCE(SUM(c.num_turns), 0) AS turns,
            COALESCE(SUM(c.input_tokens), 0) AS input_tokens,
  ...
  getSingleSessionTokenCosts: db.prepare(
...
export async function getSingleSessionCosts(sessionId) {
  return stmts.getSingleSessionCosts.get(sessionId);
}

export async function getSingleSessionTokenCosts(sessionId) {
  return stmts.getSingleSessionTokenCosts.get(sessionId);
}
