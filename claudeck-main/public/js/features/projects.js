...
export async function loadProjects() {
  try {
    ...
    // 检查嵌入模式
    const embedCwd = window.EMBED_CWD;
    if (embedCwd) {
      console.log('[Embed] Checking for project:', embedCwd);
      // 检查项目是否已在列表中
      const existingOpt = [...$.projectSelect.options].find(o => o.value === embedCwd);
      if (existingOpt) {
        $.projectSelect.value = embedCwd;
        localStorage.setItem("claudeck-cwd", embedCwd);
      } else {
        // 项目不在列表中，尝试添加它
        try {
          const result = await api.addProject(window.EMBED_PROJECT_ID || embedCwd.split(/[/\\]/).pop(), embedCwd);
          ...
          // 更新状态
          const projectsList = getState("projectsData");
          projectsList.push({ name: project.name, path: project.path });
          console.log('[Embed] Added project:', project.name, project.path);
        } catch (err) {
          console.error('[Embed] Failed to add project:', err);
          // 如果添加失败，尝试直接设置值
          const opt = document.createElement("option");
          opt.value = embedCwd;
          opt.textContent = window.EMBED_PROJECT_ID || embedCwd.split(/[/\\]/).pop();
          $.projectSelect.appendChild(opt);
          $.projectSelect.value = embedCwd;
          localStorage.setItem("claudeck-cwd", embedCwd);
        }
      }
    } else if (saved && [...$.projectSelect.options].some((o) => o.value === saved)) {
