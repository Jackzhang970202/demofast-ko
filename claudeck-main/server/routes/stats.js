...
import {
  getTotalCost, getProjectCost, getSessionCosts, getSingleSessionCosts, getSingleSessionTokenCosts, getCostTimeline, getTotalTokens, getProjectTokens,
...
router.get("/session-costs", async (req, res) => {
  try {
    const projectPath = req.query.project_path || undefined;
    const sessionId = req.query.session_id || undefined;
    if (sessionId) {
      const summary = await getSingleSessionCosts(sessionId);
      const totals = await getSingleSessionTokenCosts(sessionId);
      return res.json({ ...(summary || { id: sessionId }), ...(totals || { input_tokens: 0, output_tokens: 0, total_cost: 0 }) });
    }
    const sessions = await getSessionCosts(projectPath);
    res.json(sessions);
  } catch (err) {
