...
import { getDemoVisiblePhase, getPhaseUrl } from '@/types/workflow';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { getExecutionState } = await import('@/server/services/execution-state.service');
    const executionState = getExecutionState(projectId);
    const effectivePhase = getDemoVisiblePhase(state.phase, executionState);
    const redirect = getPhaseUrl(projectId, state.projectType, effectivePhase);

    console.log('[Demo Resume]', {
      projectId,
      workflowPhase: state.phase,
      executionStatus: executionState?.status,
      executionStage: executionState?.currentStage,
      effectivePhase,
      redirect,
    });
    ...
    return NextResponse.json({
      code: 200,
      message: 'success',
      data: { redirect, phase: effectivePhase, projectId: state.projectId },
    });
  } catch (error: any) {
